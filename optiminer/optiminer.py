            ...
            for count in range(num_block_hashes):
                ...
                if mining_condition in mining_hash:
                    if mining_condition_bin in bin_convert(mining_hash):
                        ...
                        transaction_reward = (str(block_timestamp), str(address[:56]), str(address[:56]), '%.8f' % float(0), "0", str(try_nonce))  # only this part is signed (if solo mining)
                        ...
                        if signer.verify(h, signature) == True:
                            ...
                            if pool_conf == 1:
                                app_log.warning("Mining for pool, signature check skipped")
                            else:
