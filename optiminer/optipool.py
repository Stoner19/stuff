...
def miner(q, privatekey_readable, public_key_hashed, address):
    ...
    while True:
        try:
                ...
                if mining_condition in mining_hash:
                    if mining_condition_bin in bin_convert(mining_hash):
                        ...
                        for x in removal_signature:
                            ...
                            app_log.warning(
                                "Removed a transaction with the following signature from mempool: {}".format(x))
                        mempool.commit()
                        mempool.close()
            ...
            if tries % debug_print_mod == 0:
                print(time_diff, ' for ' + str(num_block_hashes) + ' hashes',
                      float(num_block_hashes) / float(time_diff), ' hashes per sec in thread', q)
